...
def clean_and_truncate_draft(text: str, max_chars: int = 150000) -> str:
    ...
    if cutoff_idx != -1:
        logger.info(
            f"✨ 智能草稿清洗：检测到文末参考文献章节，执行切除（切除位置: {cutoff_idx}/{text_len}，节省了 {text_len - cutoff_idx} 字符）"
        )
...
class ProfileAggregator:
    ...
    def aggregate(self, features_list: List[Dict[str, Any]], user_draft_text: Optional[str] = None) -> Dict[str, Any]:
        ...
        method_distribution = {
            m: {"count": cnt, "percentage": round((cnt / total_count) * 100, 1)}
        ...
        method_avg_citations = {
            m: round(statistics.mean(cits), 1) if cits else 0 for m, cits in method_citations.items()
        }

        # 3. 定量样本量范围的统计属性 (Sample Size Stats)
        sample_sizes = [f.get("sample_size_approx", -1) for f in features_list if f.get("sample_size_approx", -1) > 0]
        sample_size_stats = {
        ...
        open_science_stats = {
            p_name: {"count": cnt, "percentage": round((cnt / total_count) * 100, 1)}
        ...
        sorted_by_citations = sorted(features_list, key=lambda x: x.get("cited_by_count", 0), reverse=True)
        ...
        logger.info(
            f"统计与对标相似度聚合计算完成。对标标杆文献: {len(most_similar_papers)} 篇，推荐引用文献: {len(recommended_references)} 篇。"
        )
